...
- Name: admin-full.spec.ts >> Admin Panel — Full Manual Test Suite >> Command Palette >> command palette lists navigation items
- Location: e2e\admin-full.spec.ts:123:9
...
Locator: locator('[role="dialog"] [cmdk-item]:has-text("Overview")').first()
Expected: attached
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('[role="dialog"] [cmdk-item]:has-text("Overview")').first()
...
- link "Sign up":
  - /url: https://nearby-koi-38.accounts.dev/sign-up?__clerk_db_jwt=dvb_3EE6xdBKMwCPsJCtkJCzMRZgm3I
