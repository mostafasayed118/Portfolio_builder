...
- Name: admin-full.spec.ts >> Admin Panel — Full Manual Test Suite >> Sidebar Navigation >> can navigate to CV page
...
Locator: locator('nav a:has-text("CV"), aside a:has-text("CV")').first()
Expected: attached
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('nav a:has-text("CV"), aside a:has-text("CV")').first()
...
- link "Sign up":
  - /url: https://nearby-koi-38.accounts.dev/sign-up?__clerk_db_jwt=dvb_3EE6yrNHVipAhLIjfocPxeJUiGD
