...
- Name: admin-full.spec.ts >> Admin Panel — Full Manual Test Suite >> Projects Manager >> projects page has add/create button
- Location: e2e\admin-full.spec.ts:260:9
...
Locator: locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New")').first()
Expected: attached
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("Add"), button:has-text("Create"), button:has-text("New")').first()
...
- link "Sign up":
  - /url: https://nearby-koi-38.accounts.dev/sign-up?__clerk_db_jwt=dvb_3EE6zjdnOYJvD6YL115OESxZQyo
