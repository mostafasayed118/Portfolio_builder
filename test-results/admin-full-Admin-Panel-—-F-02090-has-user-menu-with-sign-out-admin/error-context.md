...
- Name: admin-full.spec.ts >> Admin Panel — Full Manual Test Suite >> Layout & Sidebar >> header has user menu with sign-out
- Location: e2e\admin-full.spec.ts:85:9
...
Locator: locator('header button:has-text("@"), header [class*="user"]').first()
Expected: attached
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('header button:has-text("@"), header [class*="user"]').first()
...
- link "Sign up":
  - /url: https://nearby-koi-38.accounts.dev/sign-up?__clerk_db_jwt=dvb_3EE6xTYJh0DjAiMDBqd6J0Cy1i6
