...
- Name: admin-full.spec.ts >> Admin Panel — Full Manual Test Suite >> Sidebar Navigation >> can navigate to Certifications page
...
Locator: locator('nav a:has-text("Certifications"), aside a:has-text("Certifications")').first()
Expected: attached
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('nav a:has-text("Certifications"), aside a:has-text("Certifications")').first()
...
- link "Sign up":
  - /url: https://nearby-koi-38.accounts.dev/sign-up?__clerk_db_jwt=dvb_3EE6yqTdaIDTy0qXHcEuC4mj9uB
