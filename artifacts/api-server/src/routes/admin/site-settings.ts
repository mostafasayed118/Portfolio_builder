...
router.put("/", adminAuth, doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  ...
  let result;
  if (existing.data) {
    result = await supabase.from("site_settings").update(payload).eq("id", existing.data.id);
  } else {
    result = await supabase.from("site_settings").insert({ ...payload, is_published: true }).select("id").single();
  }
...
router.patch("/language", adminAuth, doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  const existing = await supabase.from("site_settings").select("id").limit(1).maybeSingle();
  const payload = { language: req.body.language, updated_at: new Date().toISOString() };
