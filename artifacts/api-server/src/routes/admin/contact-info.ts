...
router.put("/", adminAuth, doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  ...
  let result;
  if (existing.data) {
    result = await supabase.from("contact_info").update(payload).eq("id", existing.data.id);
  } else {
    result = await supabase.from("contact_info").insert({ ...payload, is_published: true }).select("id").single();
  }
