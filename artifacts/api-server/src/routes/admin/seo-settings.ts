...
router.put("/", adminAuth, doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  ...
  let result;
  if (existing.data) {
    result = await supabase.from("seo_settings").update(payload).eq("id", existing.data.id);
  } else {
    result = await supabase.from("seo_settings").insert({ ...payload, is_published: true }).select("id").single();
  }
