...
router.get("/", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const data = await listSectionSettings(supabase);
    return ok(res, data);
  } catch (err: unknown) {
...
router.put("/:id", validateParamId, doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const matched = await updateSectionSetting(supabase, id, result.data);
...
router.post("/reorder", doubleCsrfProtection, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    await reorderSectionSettings(supabase, result.data);
    return ok(res, undefined);
  } catch (err: unknown) {
    logSupabaseError(req, {
