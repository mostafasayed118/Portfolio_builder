...
const ProjectCard = memo(function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className="masonry-item group relative cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-2xl"
      data-testid={`card-project-${project.id}`} onClick={handleClick} role="link" tabIndex={0}
      aria-label={`View details for ${project.title}`}
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setLocation(`/projects/${slug}`); } }}>
      <div className="glass rounded-2xl border border-border/60 hover:border-primary/30 transition-all duration-500 hover:shadow-[var(--shadow-card)] hover:-translate-y-1 relative overflow-hidden">
        <div className="absolute top-3 left-3 z-20">
          <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border backdrop-blur-sm ${catConfig.color}`}>
            <catConfig.icon className="h-3 w-3" />{catConfig.label}
          </span>
        </div>
        {project.imageId && (
          <div className="overflow-hidden">
            <OptimizedImage src={project.imageId} alt={project.title} variants={project.imageVariants}
              width={800} height={450} className="h-48 w-full transition-transform duration-700 group-hover:scale-110" loading="lazy" />
          </div>
        )}
        ...
        <div className="p-5 relative">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              ...
              <h3 className="font-display font-semibold text-sm text-foreground leading-snug group-hover:text-primary transition-colors">{project.title}</h3>
            </div>
            <div className="flex gap-1 shrink-0 ml-2">
              {project.liveUrl && (
                <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  aria-label="Live site" onClick={(e) => e.stopPropagation()}>
              ...
              <a href={project.githubUrl} target="_blank" rel="noopener noreferrer"
                ...
                aria-label="View on GitHub" data-testid={`link-github-project-${project.id}`}
                onClick={(e) => e.stopPropagation()}>
                <Github className="h-4 w-4" />
              </a>
            </div>
          </div>
          <p className="text-xs text-muted-foreground leading-relaxed mb-4 line-clamp-3">{description}</p>
          {project.metrics && project.metrics.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {project.metrics.map((m) => (
                <span key={m} className="text-xs text-accent font-medium bg-accent/10 px-2 py-0.5 rounded-md border border-accent/20">{m}</span>
              ))}
            </div>
          )}
