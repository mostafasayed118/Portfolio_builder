...
export default function HeroEditor() {
  ...
  const { register, control, handleSubmit, reset, setValue, formState: { isDirty } } = useForm<HeroFormData>({
    defaultValues: {
      name: "",
      typewriter_lines: [""],
      subtitle: "",
  ...
  useEffect(() => {
    if (heroData) {
      reset({
        ...
        site_name: heroData.site_name || "",
        tagline: heroData.tagline || "",
        logo_url: heroData.logo_url || "",
        favicon_url: heroData.favicon_url || "",
        social_links: {
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <ImageUpload
                bucket="avatars"
                value={watchedData.avatar_url ?? ""}
                onChange={(url) => setValue("avatar_url", url, { shouldDirty: true })}
                label="Profile Photo"
                hint="Recommended: square image, min 400×400px"
                previewSize="md"
              />
              ...
              <ImageUpload
                bucket="documents"
                accept=".pdf"
                value={watchedData.cv_url ?? ""}
                onChange={(url) => setValue("cv_url", url, { shouldDirty: true })}
                label="CV / Resume (PDF)"
                hint="Upload your CV as a PDF file"
                showPreview={false}
              />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input {...register("site_name")} placeholder="e.g. Mustafa Sayed" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Tagline</label>
                <Input {...register("tagline")} placeholder="e.g. Data Engineer · Cairo" />
              </div>
              <ImageUpload
                bucket="avatars"
                value={watchedData.logo_url ?? ""}
                onChange={(url) => setValue("logo_url", url, { shouldDirty: true })}
                label="Logo"
                hint="Displayed in navbar. Recommended: SVG or PNG, transparent background"
                previewSize="sm"
              />
              <ImageUpload
                bucket="avatars"
                value={watchedData.favicon_url ?? ""}
                onChange={(url) => setValue("favicon_url", url, { shouldDirty: true })}
                label="Favicon"
                hint="Browser tab icon. Recommended: 32×32px PNG"
                previewSize="sm"
              />
            </CardContent>
          </Card>
          ...
          <div className="sticky top-4">
            ...
            <Card>
              <CardContent className="pt-6">
                <LivePreview data={watchedData as unknown as Partial<HeroFormData>} />
              </CardContent>
            </Card>
