...
> Admin uses the public **anon key** via `@workspace/supabase/client`. The
> Supabase service-role key is server-side only (api-server) and must never be
> added here — any `VITE_`-prefixed variable is bundled into the client.
