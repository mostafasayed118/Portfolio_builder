 ...
 * case-insensitive, but the backend's `adminAuth.ts` reads the header with
 * a specific casing, and many proxies / loggers are case-sensitive in their
 * output. Keeping the name as a single source of truth here prevents the kind
 * of "looks correct in DevTools but rejected by the server" drift.
 */
...
/**
 * Maximum number of 401 retry attempts per request. After the first 401, we
 * force-refresh the Clerk token (bypassing the JWT cache) and retry once. A
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (withAuth && res.status === 401 && retryCount < MAX_401_RETRIES) {
        if (import.meta.env.DEV) {
          logInfo(
            `[api-client] Server returned 401 (attempt ${retryCount + 1}/${MAX_401_RETRIES + 1}) — force-refreshing token and retrying.`,
            "api-client",
            { method, url },
          );
        }
      ...
      if (withAuth && res.status === 401) {
        if (import.meta.env.DEV) {
          logError(
            "[api-client] Server returned 401 after refresh attempt — token is genuinely expired. Firing auth-missing handler.",
