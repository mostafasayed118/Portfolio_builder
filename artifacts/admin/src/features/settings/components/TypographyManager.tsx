...
type TypoData = {
  ...
  base_font_size: string; line_height: string; letter_spacing: string;
  heading_scale: string; font_weight_body: string; font_weight_heading: string;
};

const DEFAULTS: TypoData = {
  body_font: "Spline Sans", display_font: "Unbounded",
  ...
  base_font_size: "16px", line_height: "1.6", letter_spacing: "0em",
  heading_scale: "1.25", font_weight_body: "400", font_weight_heading: "700",
};
...
export default function TypographyManager() {
  ...
  const { data: typoData, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => { const res = await api.typographySettings.get(); if (!res.success) throw new Error(res.message); return res.data; },
  });
  ...
  useEffect(() => {
    if (typoData) {
      setTypo({
        body_font: typoData.body_font ?? DEFAULTS.body_font, display_font: typoData.display_font ?? DEFAULTS.display_font,
        body_font_url: typoData.body_font_url ?? DEFAULTS.body_font_url, display_font_url: typoData.display_font_url ?? DEFAULTS.display_font_url,
        base_font_size: typoData.base_font_size ?? DEFAULTS.base_font_size, line_height: typoData.line_height ?? DEFAULTS.line_height,
        letter_spacing: typoData.letter_spacing ?? DEFAULTS.letter_spacing, heading_scale: typoData.heading_scale ?? DEFAULTS.heading_scale,
        font_weight_body: typoData.font_weight_body ?? DEFAULTS.font_weight_body, font_weight_heading: typoData.font_weight_heading ?? DEFAULTS.font_weight_heading,
      });
    }
  }, [typoData]);

  const set = (key: keyof TypoData, val: string) => setTypo(t => ({ ...t, [key]: val }));
  const applyPreset = (preset: typeof PRESET_PAIRS[0]) => setTypo(t => ({ ...t, display_font: preset.display, body_font: preset.body, display_font_url: preset.displayUrl, body_font_url: preset.bodyUrl }));

  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.typographySettings.update({ body_font: typo.body_font, display_font: typo.display_font, body_font_url: typo.body_font_url || null, display_font_url: typo.display_font_url || null, base_font_size: typo.base_font_size, line_height: typo.line_height, letter_spacing: typo.letter_spacing, heading_scale: typo.heading_scale, font_weight_body: typo.font_weight_body, font_weight_heading: typo.font_weight_heading });
    ...
    } catch (err) { logError("Failed to save typography settings", err, "TypographyManager"); toast({ title: "Save failed", variant: "destructive" }); }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Typography Manager</h1><p className="text-sm text-muted-foreground mt-0.5">Control fonts, sizes, and text rhythm across the portfolio.</p></div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => { setTypo(DEFAULTS); toast({ title: "Reset to defaults" }); }} className="min-h-[44px]"><RefreshCw size={14} className="mr-1.5" /> Reset</Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-2" role="radiogroup" aria-label="Font pair presets">
              {PRESET_PAIRS.map((p) => (
                <button key={p.display} type="button" role="radio" aria-checked={typo.display_font === p.display} aria-label={`Select ${p.display} + ${p.body} font preset`} onClick={() => applyPreset(p)}
                  ...
                  <div className="font-semibold text-sm text-foreground">{p.display}</div><div className="text-xs text-muted-foreground">+ {p.body}</div>
                </button>
              ))}
            </CardContent></Card>
          <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Custom Fonts</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {[{ key: "display_font" as const, urlKey: "display_font_url" as const, label: "Display / Heading Font" }, { key: "body_font" as const, urlKey: "body_font_url" as const, label: "Body Font" }].map(({ key, urlKey, label }) => (
                <div key={key} className="space-y-1.5"><Label className="text-xs">{label}</Label>
                  ...
                  <Input value={typo[urlKey]} onChange={e => set(urlKey, e.target.value)} placeholder="Google Fonts URL (optional)" className="h-8 text-xs font-mono text-muted-foreground" /></div>
              ))}
            </CardContent></Card>
          <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Scale & Rhythm</CardTitle></CardHeader>
            <CardContent className="space-y-5">
              <div><div className="flex justify-between mb-2"><Label className="text-xs">Base Font Size</Label><span className="text-xs text-muted-foreground font-mono">{typo.base_font_size}</span></div>
                <Slider value={[parseFloat(typo.base_font_size)]} min={12} max={22} step={0.5} onValueChange={([v]) => set("base_font_size", `${v}px`)} /></div>
              <div><div className="flex justify-between mb-2"><Label className="text-xs">Line Height</Label><span className="text-xs text-muted-foreground font-mono">{typo.line_height}</span></div>
                <Slider value={[parseFloat(typo.line_height) * 10]} min={12} max={22} step={1} onValueChange={([v]) => set("line_height", (v / 10).toFixed(1))} /></div>
        ...
        <TypographyPreview bodyFont={typo.body_font} displayFont={typo.display_font} baseFontSize={typo.base_font_size} lineHeight={typo.line_height} letterSpacing={typo.letter_spacing} fontWeightBody={typo.font_weight_body} fontWeightHeading={typo.font_weight_heading} />
      </div>
    </div>
  );
}
