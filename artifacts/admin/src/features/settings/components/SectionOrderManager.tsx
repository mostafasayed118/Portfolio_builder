...
export default function SectionOrderManager() {
  ...
  const handleSave = async () => {
    setSaving(true);
    // Track visibility updates already persisted so a mid-loop failure
    // can be rolled back instead of leaving a partial save.
    const applied: { id: string; prev: boolean }[] = [];
    try {
      ...
      for (const v of visChanges) {
        const prev = originalDataRef.current.find(o => o.id === v.id)?.is_visible ?? v.is_visible;
        ...
        applied.push({ id: v.id, prev });
      }

      originalDataRef.current = [...sections];
      toast({ title: "Section order saved" });
    } catch (err) {
      // Roll back already-applied visibility changes (best effort)
      for (let i = applied.length - 1; i >= 0; i--) {
        try {
          await api.sectionSettings.update(applied[i].id, { is_visible: applied[i].prev });
        } catch { /* best effort rollback */ }
      }
      logError("Failed to save section order", err, "SectionOrderManager");
