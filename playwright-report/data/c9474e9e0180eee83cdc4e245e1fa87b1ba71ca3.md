...
Call log:
  ...
  - waiting for locator('button[aria-label*="theme" i], button[aria-label*="dark" i], button[aria-label*="light" i]').first()
    12 × locator resolved to <button aria-label="Toggle theme" data-testid="btn-theme-toggle" class="h-9 w-9 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-all">…</button>
