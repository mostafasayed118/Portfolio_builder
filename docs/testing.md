...
> **Last updated:** 2026-09-14 (merged the duplicate `docs/TESTING_GUIDE.md` into this file; counts verified: 1,500+ test cases across ~195 test files).

This guide focuses on **how to be productive with the test suite day-to-day**. The deeper "why" lives in the in-file test docstrings and the historical record in [`docs/archive/`](./archive/).

## TL;DR

```bash
# All tests (admin + portfolio + api-server + libs + scripts)
pnpm run test
...
# API server: dedicated commands
pnpm --filter @workspace/api-server test          # run all API tests once
...
| ---------------- | ------------------------------------ | -------------------------------------------------------------- | ------------------------------------------------------------------- |
| Component / hook | **Vitest** + Testing Library + jsdom | `artifacts/{admin,portfolio}/src/test/`, `src/hooks/*.test.ts` | All admin features, all portfolio features, hooks, validation, a11y |
...
The root `vitest.config.ts` defines 8 projects:

| Project    | Root                   | Environment |
| ---------- | ---------------------- | ----------- |
| portfolio  | `artifacts/portfolio`  | jsdom       |
| admin      | `artifacts/admin`      | jsdom       |
| api-server | `artifacts/api-server` | node        |
| logging    | `lib/logging`          | node        |
| validation | `lib/validation`       | node        |
| api-zod    | `lib/api-zod`          | node        |
| db         | `lib/db`               | node        |
...
- [ ] Does the test still pass if you introduce a real bug in the unit under test? → **Mutate the source and re-run. If the test still passes, it's hallucinated.**
...
// Uses test-utils for a mock Supabase client
```
...
2. **component-tests** — `pnpm run test` against the full monorepo
