-- Fix reorder_sections RPC: add admin check to SECURITY DEFINER function
-- Without this check, any authenticated user could reorder sections
...
COMMENT ON FUNCTION reorder_sections(UUID[], INTEGER[])
  IS 'Atomically reorders multiple sections in a single transaction. Admin-only.';
