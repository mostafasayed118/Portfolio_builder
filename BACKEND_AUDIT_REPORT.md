...
| middleware/csrf.ts       | 0% → covered     | `csrf.unit.test.ts` (8 unit tests) exercises the real csrf-csrf wrapper                                                                           |
| middleware/validate.ts   | 17.44% → covered | renamed to `validateUuid.ts`; 7 tests incl. empty-string and missing-param branches                                                               |
...
5. ~~**middleware/validate.ts** (17.44%) — Schema validation untested~~ — renamed to `validateUuid.ts`; now covered (7 tests)
6. ~~**middleware/csrf.ts** (0%) — Only tested via integration~~ — now covered (`csrf.unit.test.ts`, 8 unit tests against the real csrf-csrf wrapper)
...
| --- | -------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------- |
| L1  | Remove placeholder migrations          | ✅ Resolved           | Re-audit (2026-08): migrations 010/016/017/018/019 contain real SQL — not placeholders; left in place |
...
6. **Input validation** — Custom framework in `validateUuid.ts` + Zod on all write routes
...
_Report generated by automated deep-scan. All findings verified against source code._
